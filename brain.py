...
from tools import (cancel_popup, check_if_current_week_has_played, check_if_current_week_islive, 
                   check_if_last_result_equal_input, clear_bet_slip, reload_result_page,save_page,confirm_outcome,send_email)
import os
...
class PlayGame:
    def __init__(self,driver:object,market:str) -> None:
        """" To handle the Game Play like: choose_market, select_stake_option, place_the_bet. It takes a driver instance as first argument"""
    ...
    def choose_market(self):
        ...
        try:
            if check_if_current_week_islive(self.browser):
                time.sleep(40)
            more_markets_button=self.wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR,'[data-testid="market-dropdown-more-markets"]')))
            more_markets_button.click()
        except (StaleElementReferenceException, ElementClickInterceptedException, TimeoutException):
                self.browser.execute_script(f"window.scrollTo(0, 20);")
                if check_if_current_week_islive(self.browser):
                    time.sleep(40)
                more_markets_button=self.wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR,'[data-testid="market-dropdown-more-markets"]')))
                more_markets_button.click()    
        ...
        time.sleep(0.5)
        try:
            close_more_markets_button=self.wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR,'[data-testid=market-dropdown-close-markets]')))
    ...
    def select_stake_options(self,week:str,previous_week_selected:str)->str:
        ...
        for n in range(start,len(available_games[:end])):
            ...
            try:
                ...
                self.browser.execute_script(f"window.scrollTo(0, {window_height*attempt1});") # To Scroll to where the element can be clicked()
    ...
    def place_the_bet(self,amount:int,test:bool):
        ...
        time.sleep(1)
        betslip_button=self.wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR,'[data-testid="nav-bar-betslip"]')))
        betslip_button.click()
        time.sleep(1)
        # identify and click the singles tab option
        singles_button=self.wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR,'[data-testid="groupings-tab-singles"]')))
        ...
        if test:
            ...
            try:
                continue_betting_button=self.wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR,'button.btn.upper.btn-transparent')))
                continue_betting_button.click    
            except (TimeoutException,NoSuchElementException):
                close_betslip_button=self.wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR,'[data-testid="coupon-close-icon"]')))
...
class CheckPattern:
    def __init__(self,driver:object,market:str) -> None:
        """ To check if the the desired pattern of the desired market has occured. It takes a driver instance as first argument """
        self.market=market
    ...
    def check_result(self,length:str,latest_week:str)->bool:
        ...
        standings_button=self.wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR,'[data-testid="results-and-standings-button"]')))
        # standings_button=self.wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR,"span.view-switch-icon")))
        standings_button.click()
        result_button=self.wait.until(EC.element_to_be_clickable((By.XPATH,"/html/body/app-root/app-wrapper/div/virtuals-league-wrapper/mobile-virtuals-soccer/mvs-virtual-league-page/div[2]/mvs-results-page/div[2]/div[2]")))
        result_button.click()
        time.sleep(5)
        ...
        if length.lower()=="all result":
            ...
            game_weeks=check_if_last_result_equal_input(self.browser,game_weeks=game_weeks,week_to_check=latest_week,
                                                        time_delay=30)
...
class LoginUser:
    def __init__(self,driver:object,username:str,password:str) -> None:
        """ To login a user with the relevant credentials.  It takes a driver instance as first argument """
    ...
    def login(self):
        ...
        time.sleep(2)
        try:
            cancel_notification_option_button=self.wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR,"button.kumulos-action-button-cancel")))
