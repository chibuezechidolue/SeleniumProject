...
def start_bot():
    ...
    if check_result['outcome'] != "":
        ...
        left=mp.Process(target=play_process,args=(que,SELECTED_MARKET,check_result,MAX_AMOUNT_LENGTH,week_to_save1,LEAGUE,0),daemon=True)
        left.start()
        if check_result['outcome']!='3 - 2' and check_result['outcome']!='2 - 3':
            right=mp.Process(target=play_process,args=(que,SELECTED_MARKET,check_result,MAX_AMOUNT_LENGTH,week_to_save1,LEAGUE,1),daemon=True)
            right.start()
        ...
        print(f"this is the result: {won}")   
        if not won:
            send_email(Email=os.environ.get("EMAIL_USERNAME"),
                    Password=os.environ.get("EMAIL_PASSWORD"),
                    Subject="YOU'VE LOST IT ALL",
